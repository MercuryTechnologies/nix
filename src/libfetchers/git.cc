...
struct GitInputScheme : InputScheme
{
    ...
    std::optional<Input> inputFromAttrs(const Attrs & attrs) const override
    {
        ...
        for (auto & [name, value] : attrs)
            if (name != "type" && name != "url" && name != "ref" && name != "rev" && name != "shallow" && name != "submodules" && name != "lastModified" && name != "revCount" && name != "narHash" && name != "allRefs" && name != "name" && name != "dirtyRev" && name != "dirtyShortRev" && name != "date")
